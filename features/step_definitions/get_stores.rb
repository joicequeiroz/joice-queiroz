...
When("I send the GET verb to search all stores") do                            
  $response = HTTParty.get("http://localhost:3030/stores", headers: @header)
  puts $response.body
end                                                                            
                                                                               
When("I send the GET verb to find stores in Minnesota") do                     
  $response = HTTParty.get("http://localhost:3030/stores?state=MN", headers: @header)
  puts $response.body
end                                                                            
                                                                               
When("I send the GET verb to find stores sell Apple products") do              
  $response = HTTParty.get("http://localhost:3030/stores?service.name=Apple Shop", headers: @header)
  puts $response.body
end                                                                            
                                                                               
When("I send the GET verb to find stores within ten miles of Beverly Hills") do
